...
$tanggal=substr($keterangan,0,10);
...
while ($row=mysql_fetch_array($query))
{
	  ...
	  echo "<tr><td>Bentuk Tes</td><td>Tertulis / Pilihan ganda</td><td>Tahun Pelajaran</td><td>$tahunajaran</td><tr>";
	  echo "</table>";
	  ...
	  $opsie=$row["opsie"];
	  $kunchi=$row["kunci"];
	  echo "<tr><td valign=top width=30% height=120>";
	  echo "<b>Kompetensi Dasar :</b>";

	  $carikd=substr($soal,0,4);
	  $querykd=mysql_query("select * from silabus_kd where kode='$carikd'", $koneksi);
	  while ($rowkd=mysql_fetch_array($querykd))
	  {
	  	$kompetensidasar=$rowkd["keterangan"];
	  }
	  echo "<br>$kompetensidasar";

	  echo "</td>";

	  echo "<td rowspan=3 valign=top>";
	  echo "<table width=100%>";
	  echo "<tr><td width=10%>No. Soal</td><td width=10%>Kunci Jawaban</td><td rowspan=2 width=15%>Buku Sumber </td><td rowspan=2>: $sumber</tr>";
	  echo "<tr><td><center>$no</td><td><center>$kunchi</td></tr>";
	  echo "</table>";
	  ...
	  echo "<tr><td valign=top height=120>";
	  echo "<b>Materi :</b>";

	  $carimateri=substr($soal,0,5);
	  $querymateri=mysql_query("select * from silabus_materi where kode='$carimateri'", $koneksi);
	  while ($rowmateri=mysql_fetch_array($querymateri))
	  {
	  	$smateri=$rowmateri["keterangan"];
	  }
	  echo "<br>$smateri";
	  ...
	  echo "<tr><td valign=top height=120>";
	  echo "<b>Indikator Soal :</b>";

	  $cariindikator=substr($soal,0,6);
	  $queryindikator=mysql_query("select * from silabus_indikator where kode='$cariindikator'", $koneksi);
	  while ($rowindikator=mysql_fetch_array($queryindikator))
	  {
	  	$indikator=$rowindikator["keterangan"];
	  }
	  echo "<br>$indikator";
...
#	  echo "</table>";
	  ...
	  echo "<td colspan=5><center>Proporsi jawaban pada pilihan</td>";
	  echo "<td rowspan=2>Keterangan</td></tr>";

	  echo "<tr><td>A</td><td>B</td><td>C</td><td>D</td><td>Omit</td></tr>";
	  echo "<tr><td>$untuk</td><td>$tgujian</td><td>$jumlahtestee</td><td>$tk ($ptk)</td>";
	  echo "<td>$pdp</td><td>$tslha%</td><td>$tslhb%</td><td>$tslhc%</td><td>$tslhd%</td><td>$tslhk%</td><td>$qdp</td></tr>";

#	  echo "<tr><td width=5%><center>A</td><td width=5%><center>B</td><td width=5%><center>C</td><td width=5%><center>D</td><td width=5%><center>Omit</td></tr>";
#	  echo "<tr><td>$untuk</td><td>$tanggal</td><td> </td><td> </td>";
#	  echo "<td> </td><td> </td><td> </td><td> </td><td> </td><td> </td><td> </td></tr>";
	  echo "</table><br>";

	  echo "<p class=pisah></p>";
}
echo "</table>";
